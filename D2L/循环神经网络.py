...
class ScriptAnimator:
    """d2l.Animator 的终端脚本版:实时刷新窗口,结束后保存png(不依赖Jupyter)"""
    ...
    def add(self, x, y):
        ...
        for i, (a, b) in enumerate(zip(x, y)):
            if a is not None and b is not None:
                self.X[i].append(a)
                self.Y[i].append(b)
        # 重绘并刷新窗口(替代 Jupyter 的 display)
        self.axes.cla()
        for x_, y_, fmt in zip(self.X, self.Y, self.fmts):
            self.axes.plot(x_, y_, fmt)
        self.config_axes()
        plt.pause(0.001)
...
def train_ch8(net, train_iter, vocab, lr, num_epochs, device, use_random_iter = False):
    ...
    animator = ScriptAnimator(xlabel='epoch', ylabel='perplexity',
